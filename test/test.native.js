...
tape( 'the function computes exp(z) for pure imaginary z', opts, function test( t ) {
	...
	for ( i = 0; i < re.length; i++ ) {
		...
		t.strictEqual( realf( q ), f32( expre[ i ] ), 'returns expected value' );
		t.strictEqual( imagf( q ), f32( expim[ i ] ), 'returns expected value' );
	}
	t.end();
});
...
tape( 'the function computes exp(z) for complex z', opts, function test( t ) {
	...
	for ( i = 0; i < re.length; i++ ) {
		q = cexpf( new Complex64( re[ i ], im[ i ] ) );
		t.strictEqual( isAlmostSameValuef( realf( q ), f32( expre[ i ] ), 2 ), true, 'returns expected value' );
		t.strictEqual( isAlmostSameValuef( imagf( q ), f32( expim[ i ] ), 2 ), true, 'returns expected value' );
	}
	t.end();
});
...
tape( 'the function computes exp(z) for pure real z (positive)', opts, function test( t ) {
	var v;

	v = cexpf( new Complex64( 1.0, 0.0 ) );
	t.strictEqual( realf( v ), 2.7182817459106445, 'returns expected value' );
	t.strictEqual( isPositiveZerof( imagf( v ) ), true, 'returns expected value' );
	t.end();
});

tape( 'the function computes exp(z) for pure real z (negative)', opts, function test( t ) {
	var v;

	v = cexpf( new Complex64( -1.0, 0.0 ) );
	t.strictEqual( realf( v ), 0.3678794503211975, 'returns expected value' );
	t.strictEqual( isPositiveZerof( imagf( v ) ), true, 'returns expected value' );
	t.end();
});

tape( 'if real component is a large positive value and imaginary component is `0.0`, the function returns `+Infinity`', opts, function test( t ) {
	var v;

	v = cexpf( new Complex64( 89.0, 0.0 ) );
	t.strictEqual( realf( v ), PINF, 'returns expected value' );
	t.strictEqual( isPositiveZerof( imagf( v ) ), true, 'returns expected value' );
	t.end();
});

tape( 'if real component is `+Infinity` and imaginary component is finite and nonzero, the function returns infinite components', opts, function test( t ) {
	var v;

	v = cexpf( new Complex64( PINF, 1.0 ) );
	t.strictEqual( realf( v ), PINF, 'returns expected value' );
	t.strictEqual( imagf( v ), PINF, 'returns expected value' );
	t.end();
});

tape( 'if real component is `-Infinity` and imaginary component is `0.0`, the function returns a complex number with both components equal to `+0.0`', opts, function test( t ) {
	var v;

	v = cexpf( new Complex64( NINF, 0.0 ) );
...
tape( 'if real component is `+Infinity` and imaginary component is `-Infinity`, the function returns a complex number with the real component equal to `-Infinity` and imaginary component equal to `NaN`', opts, function test( t ) {
	...
	v = cexpf( new Complex64( PINF, NINF ) );
	t.strictEqual( realf( v ), NINF, 'returns expected value' );
	t.strictEqual( isnanf( imagf( v ) ), true, 'returns expected value' );
	t.end();
});
